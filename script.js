...
var x = setInterval(function() {
	...
	// display the result
	document.getElementById("countdown").innerHTML = `There are ${days} days, ${hours} hours, ${minutes} minutes, and ${seconds} seconds left in ${currentYear}.`;
	...
	function calculateNextPercentage(totalSecondsOfYear, secondsElapsed) {
		...
		futurePercentageDate = new Date(futurePercentageDate).toLocaleDateString('en-us', { weekday:"long", year:"numeric", month:"short", day:"numeric", hour:"numeric", minute:"numeric", second:"numeric", timeZoneName:"short"});
		return [nextPercentage, futurePercentageDate];
	}
	
	// display the result
	var nextPercentage = calculateNextPercentage(secondsOfYear, secondsElapsed);
	document.getElementById("next-percentage").innerHTML = `2024 will be ${`${nextPercentage[0]}%`} complete on ${nextPercentage[1]}.`;
